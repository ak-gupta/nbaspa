...
class AddWinProbability(Task):
    ...
    def run(self, pbp: pd.DataFrame, win_prob: pd.Series) -> pd.DataFrame:
        ...
        * ``WIN_PROBABILITY_CHANGE``
        ...
        filter = pbp.sort_values(by="EVENTNUM", ascending=True).duplicated(
            subset="TIME", keep="last"
        )
        pbp["WIN_PROBABILITY"] = np.nan
        pbp.loc[~filter, "WIN_PROBABILITY"] = pbp[~filter].merge(
            win_prob, left_on="EVENTNUM", right_index=True, how="left"
        )[win_prob.name]
        # Create a variable that represents the change in win probability over events
        pbp.loc[~pd.isnull(pbp["WIN_PROBABILITY"]), "WIN_PROBABILITY_CHANGE"] = pbp.loc[
